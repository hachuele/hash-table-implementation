...
public:
  ...
  static const int HASH_SIZE = 9973;  // a prime number
   ...
   //   number of buckets: 997 (return hashSize)
   //   number of entries: 10 (Keep Track - initialize in constructor)
   //   number of non-empty buckets: 9 (Function for loop through table)
   //   longest chain: 2 (create length of chain function, loop through table to find max)
...
//Returns the number of non-empty buckets from the hash table
int nonEmptyBuckets() const;

//Returns the size of the longest chain in the table
int longestChainNumber() const;


  unsigned int hashSize;      // size of the hash table
                              // (used in hashCode method above)
  
  int numberEntries; //Number of entries in the table
  
  ListType* hashTable; //hash table initialization
